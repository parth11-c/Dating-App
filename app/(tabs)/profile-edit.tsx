...
export default function ProfileEditScreen() {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: 12, paddingBottom: 120 }}>
      <View style={styles.card}>
        ...
        <View style={styles.segmentRow}>
          {(['male','female','non-binary','other'] as const).map(g => (
            <TouchableOpacity key={g} style={[styles.segment, profile?.gender === g && styles.segmentActive]} onPress={() => setProfile((p: any) => ({ ...(p || {}), gender: g }))}>
        ...
        <View style={{ flexDirection: 'row', gap: 8 }}>
          <View style={{ flex: 1 }}>
            <Picker selectedValue={dobDay} onValueChange={(v) => { setDobDay(String(v)); setProfile((p: any) => ({ ...(p || {}), date_of_birth: `${dobYear}-${String(dobMonth).padStart(2,'0')}-${String(v).padStart(2,'0')}`})); }} style={styles.input as any}>
          ...
          <View style={{ flex: 1 }}>
            <Picker selectedValue={dobMonth} onValueChange={(v) => { setDobMonth(String(v)); setProfile((p: any) => ({ ...(p || {}), date_of_birth: `${dobYear}-${String(v).padStart(2,'0')}-${String(dobDay).padStart(2,'0')}`})); }} style={styles.input as any}>
          ...
          <View style={{ flex: 1.3 }}>
            <Picker selectedValue={dobYear} onValueChange={(v) => { setDobYear(String(v)); setProfile((p: any) => ({ ...(p || {}), date_of_birth: `${String(v)}-${String(dobMonth).padStart(2,'0')}-${String(dobDay).padStart(2,'0')}`})); }} style={styles.input as any}>
      ...
      <View style={styles.card}>
        ...
        {interestNames.length === 0 ? (
          <Text style={styles.mutedSmall}>You haven't added any interests yet.</Text>
        ) : (
          <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 6 }}>
            {interestNames.map((n) => (
              <View key={n} style={styles.interestChip}>
                <Ionicons name={interestIcon(n) as any} size={14} color="#bbb" style={{ marginRight: 6 }} />
                <Text style={styles.interestChipText}>{n}</Text>
              </View>
            ))}
          </View>
        )}
        <TouchableOpacity style={[styles.btn, { marginTop: 10, alignSelf: 'flex-start' }]} onPress={() => router.push('/(tabs)/interests' as any)}>
          <Ionicons name="add-outline" size={16} color="#000" />
          <Text style={styles.btnText}>Edit interests</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.card}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginTop: 6 }}>
          <View style={{ flex: 1 }}>
            ...
            <Slider minimumValue={18} maximumValue={prefAgeMax} step={1} value={prefAgeMin} onValueChange={setPrefAgeMin} minimumTrackTintColor="#cce6ff" maximumTrackTintColor="#333" thumbTintColor="#4da3ff" />
          </View>
          <View style={{ flex: 1 }}>
            <Text style={styles.mutedSmall}>Max</Text>
            <Slider minimumValue={prefAgeMin} maximumValue={99} step={1} value={prefAgeMax} onValueChange={setPrefAgeMax} minimumTrackTintColor="#cce6ff" maximumTrackTintColor="#333" thumbTintColor="#4da3ff" />
          </View>
        </View>
      </View>

      <View style={styles.card}>
        <Text style={styles.cardTitle}>Photos (exactly 4)</Text>
        <Text style={styles.mutedSmall}>Photos added: {photos.length}/4</Text>
        <View style={{ flexDirection: 'row', gap: 12, marginTop: 10 }}>
          <TouchableOpacity style={styles.btn} onPress={pickFromLibrary} disabled={photos.length >= 4}>
            <Ionicons name="image-outline" size={16} color="#000" />
            <Text style={styles.btnText}>Pick</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.btn} onPress={takePhoto} disabled={photos.length >= 4}>
        ...
        <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10, marginTop: 12 }}>
          {photos.map((p) => (
            <View key={p.id} style={{ width: '31%', aspectRatio: 1, borderRadius: 10, overflow: 'hidden', borderWidth: 1, borderColor: '#222', position: 'relative' }}>
...
function Chip({ label, active, onPress }: { label: string; active: boolean; onPress: () => void }) {
  ...
  return (
    <Pressable onPressIn={onIn} onPressOut={onOut} onPress={onPress} style={{ borderRadius: 18 }}>
      <Animated.View style={[{ backgroundColor: '#0f0f10', borderColor: '#222', borderWidth: 1, paddingHorizontal: 12, paddingVertical: 10, borderRadius: 18, flexDirection: 'row', alignItems: 'center' }, active && { borderColor: '#fff', backgroundColor: '#141416' }, { transform: [{ scale }] }]}> 
        <Ionicons name="book-outline" size={14} color={active ? '#fff' : '#bbb'} style={{ marginRight: 6 }} />
        <Text style={[{ color: '#c7c7c7', fontWeight: '700' }, active && { color: '#fff' }]}>{label}</Text>
      </Animated.View>
    </Pressable>
  );
}
